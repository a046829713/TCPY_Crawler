...
class TCoreZMQ():
    ...
    def Connect(self, port):
        self.lock.acquire()
        login_obj = {"Request": "LOGIN", "Param": {
            "SystemName": self.appid, "ServiceKey": self.ServiceKey}}
    ...
    def QueryInstrumentInfo(self, sessionKey, symbol):
        self.lock.acquire()
        obj = {"Request": "QUERYINSTRUMENTINFO",
               "SessionKey": sessionKey, "Symbol": symbol}
        self.socket.send_string(json.dumps(obj))
    ...
    def QueryAllInstrumentInfo(self, sessionKey, type):
        self.lock.acquire()
        obj = {"Request": "QUERYALLINSTRUMENT",
               "SessionKey": sessionKey, "Type": type}
        self.socket.send_string(json.dumps(obj))
        message = self.socket.recv()[:-1]
        data = json.loads(message)
        self.lock.release()
...
class TradeAPI(TCoreZMQ):
    ...
    def QryReport(self, sessionKey, qryIndex):
        self.lock.acquire()
        obj = {"Request": "RESTOREREPORT",
               "SessionKey": sessionKey, "QryIndex": qryIndex}
        self.socket.send_string(json.dumps(obj))
        message = self.socket.recv()[:-1]
        data = json.loads(message)
        self.lock.release()
    ...
    def QryMargin(self, sessionKey, accountMask):
        self.lock.acquire()
        obj = {"Request": "MARGINS", "SessionKey": sessionKey,
    ...
    def QryPosition(self, sessionKey, accountMask, qryIndex):
        self.lock.acquire()
        obj = {"Request": "POSITIONS", "SessionKey": sessionKey,
               "AccountMask": accountMask, "QryIndex": qryIndex}
        self.socket.send_string(json.dumps(obj))
        message = self.socket.recv()[:-1]
        data = json.loads(message)
        self.lock.release()
...
class QuoteAPI(TCoreZMQ):
    ...
    def SubGreeks(self, sessionKey, symbol, greeksType="REAL"):
        self.lock.acquire()
        obj = {"Request": "SUBQUOTE", "SessionKey": sessionKey}
        obj["Param"] = {"Symbol": symbol,
                        "SubDataType": "GREEKS", "GreeksType": greeksType}
        self.socket.send_string(json.dumps(obj))
        message = self.socket.recv()[:-1]
        data = json.loads(message)
        self.lock.release()
    ...
    def SubHistory(self, sessionKey, symbol, type, startTime, endTime):
        """ 後來改用 with的方式處理避免線程卡住

        Args:
            sessionKey (_type_): _description_
        ...
        try:
            with self.lock:
                obj = {"Request": "SUBQUOTE", "SessionKey": sessionKey}
                obj["Param"] = {"Symbol": symbol, "SubDataType": type,
                                "StartTime": startTime, "EndTime": endTime}
                self.socket.send_string(json.dumps(obj))
                message = self.socket.recv()[:-1]
                data = json.loads(message)
                
                Debug_tool.debug.record_msg(f"線程名稱:{threading.current_thread().name},訂閱成功")
                return data
        except Exception as e:
            Debug_tool.debug.print_info(error_msg=f"線程名稱:{threading.current_thread().name},訂閱失敗")
            return e
    ...
    # 分页获取订阅的历史数据
    def GetHistory(self, sessionKey, symbol, type, startTime, endTime, qryIndex,status_code):
        try:
            with self.lock:
                obj = {"Request": "GETHISDATA", "SessionKey": sessionKey}
                obj["Param"] = {"Symbol": symbol, "SubDataType": type,
                                "StartTime": startTime, "EndTime": endTime, "QryIndex": qryIndex}
                self.socket.send_string(json.dumps(obj))
                message = (self.socket.recv()[:-1]).decode("utf-8")
                index = re.search(":", message).span()[1]  # filter
                message = message[index:]
                message = json.loads(message)
                Debug_tool.debug.record_msg(f"GetHistory-取得回傳資料,線程名稱:{threading.current_thread().name},狀態碼:{status_code}")
                
                return message
        except Exception as e:
            Debug_tool.debug.print_info(error_msg=f"線程名稱:{threading.current_thread().name},GetHistory-訂閱歷史數據失敗,qryIndex:{qryIndex},狀態碼:{status_code}")
            return e

class KeepAliveHelper():
    def __init__(self, subPort, session, objZMQ):
        threading.Thread(target=self.ThreadProcess, args=(
            subPort, session, objZMQ)).start()
